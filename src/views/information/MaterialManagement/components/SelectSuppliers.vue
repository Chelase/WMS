<script setup lang="ts">
    ...
    //store
    const getQueryData = useSupplierStore()

    const emit = defineEmits(['selectionstwo'])
    ...
    const getQueryDatas = async () => {
        // store 表格列表get方法
        await getQueryData.getSupplierData(getneedData.value)
        //store 表格列表存值
        tableData.value = getQueryData.supplierData
        tableDataNum.value = getQueryData.supplierTotal
    ...
    const dialogTableVisible = inject('showSupplier')
    //存储单选框的值
    ...
    //选择
    const selectionfun = () => {
        emit('selectionstwo', selections.value)
        dialogTableVisible.value = false
    }
</script>
<template>
    <el-dialog v-model="dialogTableVisible" title="供应商选择" class="w-65%">
